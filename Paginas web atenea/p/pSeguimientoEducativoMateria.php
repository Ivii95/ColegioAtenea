...
if($numeroPaginas>1){
    ...
    $anterior=$page-1;
    if($page>1){
        $ant="<a href=\"".$estaPage."?p=".$anterior."&ma=".$_GET["ma"]."&m=true\" target=\"_parent\">&laquo;Anterior&nbsp;</a>";
    }else{
        $ant="";
    }
    $paginacion="<div class=\"paginacion\">".$ant."Pág. ".$page." / ".$numeroPaginas."&nbsp;&nbsp;";
    if($page<$numeroPaginas){
        $paginacion.="<a href=\"".$estaPage."?p=".$siguiente."&ma=".$_GET["ma"]."&m=true\" target=\"_parent\">Siguiente&raquo;</a></div>\n";
    }else{
        $paginacion.="</div>\n";
    }
}else{
