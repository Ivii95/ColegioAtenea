...
//
$makeEjercicios=$bd->query("select * from parciales where idCurso='".$filaCurso["id"]."' and idMateria='".$_GET["ma"]."' order by fecha desc  limit ".$limiteInferior.",".$limiteSuperior.";");
$filaEjercicios=$bd->fil($makeEjercicios);
$colEjercicios=$bd->col($makeEjercicios);

$makeTotalEjercicios=$bd->query("select * from parciales where idCurso='".$filaCurso["id"]."' and idMateria='".$_GET["ma"]."';");
...
if($colEjercicios>0){
    ...
    $b.="<div class=\"foto_profesor\"><img class='sombrasimagenes' src=\"https://colegioatenea.es/".$rutaImagenProfesor."\" oncopy=\"alert('Opcion deshabilitada');return false\" oncontextmenu=\"alert('Opcion deshabilitada');return false\" alt=\"profesor\" width=\"100px\"/></div>\n";
    ...
    do{
        ...
        $b.="<p>Descripción: ".$filaEjercicios["descripcion"]."</p>";
        $b.="<p>Fecha: ".$fecha." Hora: ".$filaEjercicios["hora"]."</p>";
        $b.="<p>Calificación: ".$nota."</p>";
        $b.="<p>Observaciones: ".$observaciones."</p>";
        $b.="</div>";
