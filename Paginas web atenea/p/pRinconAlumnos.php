...
$make=$bd->query("select * from publicaciones where categoria='alumno' and month(fecha)='".$mesActual."' order by fecha desc limit ".$limiteInferior.",".$limiteSuperior.";");
$fila=$bd->fil($make);
$cols=$bd->col($make);

$makeTotal=$bd->query("select * from publicaciones where categoria='alumno' and month(fecha)='".$mesActual."' order by fecha desc;");
...
if($cols>0){ //si existen publicaciones de este mes...
    do{
        ...
        //archivo
        $dira="admin/archivosPubli/alumno/".$fila["id"];
        if(is_dir($dira)){
            $opena=opendir($dira);
        ...
        //imagen
        $dira2="admin/imagenesPubli/alumno/".$fila["id"];
        if(is_dir($dira2)){
            ...
            $rr2="<div class=\"fotos_rincon\"><a href=\"".$rutaArchivo2."\" target=\"_blank\" title=\"Imagen de la publicación\"><img src=\"".$rutaArchivo2."\" title=\"Imagen de la publicación\" border=\"0\" alt=\"Imagen de la publicación\" width=\"".$anchop."\"/></a></div>\n";
        }else{
            $rr2="<div class=\"fotos_rincon\"><img src=\"imagenes/sin_foto.gif\" title=\"Imagen no disponible\" alt=\"Imagen no disponible\" width=\"188\" /></div>\n";
        }
        //enlace
        if($fila["enlaces"]!=""){
    ...
    $salida="Aún no hay publicaciones en este mes";
}
//paginaciones
if($numeroPaginas>1){
    $siguiente=$page+1;
...
//    }
//    $cons="select * from publicaciones where categoria='alumno' ".$repre2."".$repre.";";
...
for($i=0;$i<12;$i++){
    ...
    if($_GET["t"]=="1"){echo $mesHoy."*";}
    $makeMes=$bd->query("select * from publicaciones where categoria='alumno' and month(fecha)='".$mesHoy."' order by fecha asc;");
    $filaMes=$bd->fil($makeMes);
