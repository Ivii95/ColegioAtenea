...
$make=$bd->query("select * from publicaciones where categoria='alumno' and month(fecha)='".$mesActual."' and year(fecha)='".$aaa."' order by fecha desc limit ".$limiteInferior.",".$limiteSuperior.";");
$fila=$bd->fil($make);
$cols=$bd->col($make);

$makeTotal=$bd->query("select * from publicaciones where categoria='alumno' and month(fecha)='".$mesActual."'  and year(fecha)='".$aaa."';");
...
if($cols>0){ //si existen publicaciones de este mes...
    do{
        ...
        if(is_dir($dira2)){
            ...
            $rr2="<div class=\"fotos_rincon\"><a href=\"".$rutaArchivo2."\" title=\"Imagen de la publicación\" target=\"_blank\"><img src=\"".$rutaArchivo2."\" border=\"0\" title=\"Imagen de la publicación\" alt=\"Imagen de la publicación\" width=\"".$anchop."\"/></a></div>\n";
        }else{
