...
$make=$bd->query("select * from publicaciones where categoria='padre' and month(fecha)='".$mesActual."' order by fecha desc limit ".$limiteInferior.",".$limiteSuperior.";");
$fila=$bd->fil($make);
$cols=$bd->col($make);

$makeTotal=$bd->query("select * from publicaciones where categoria='padre' and month(fecha)='".$mesActual."' order by fecha desc;");
...
if($cols>0){ //si existen publicaciones de este mes...
    do{
        ...
        //archivo
        $dira="admin/archivosPubli/padre/".$fila["id"];
        if(is_dir($dira)){
            $opena=opendir($dira);
		...
		//imagen
        $dira2="admin/imagenesPubli/padre/".$fila["id"];
        if(is_dir($dira2)){
