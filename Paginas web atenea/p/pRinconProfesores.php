...
$make=$bd->query("select * from publicaciones where categoria='profesor' and month(fecha)='".$mesActual."' order by fecha asc limit ".$limiteInferior.",".$limiteSuperior.";");
$fila=$bd->fil($make);
$cols=$bd->col($make);

$makeTotal=$bd->query("select * from publicaciones where categoria='profesor' and month(fecha)='".$mesActual."' order by fecha asc;");
...
if($cols>0){ //si existen publicaciones de este mes...
    do{
        ...
        if(is_dir($dira2)){
           ...
           $rr2="<div class=\"fotos_rincon\"><a href=\"".$rutaArchivo2."\" target=\"_blank\" title=\"Imagen de la publicación\"><img src=\"".$rutaArchivo2."\" border=\"0\" title=\"Imagen de la publicación\" alt=\"Imagen de la publicación\" width=\"".$anchop."\"/></a></div>\n";
        }else{
